...
- Every active timer shows its own countdown to the left of the icon, the most recent one right next to it; double-click or right-click a countdown to delete that timer
- Sound (Hero), the same bloom-and-ripple animation at the menu icon, plus a user notification when a timer fires, with a floating alert as fallback
...
The icon and countdowns share one hit-transparent status-item content view. Countdown strings are measured and drawn directly with the same attributed-string API, while the template icon remains an `NSImageView`. Drawing reads the actual bounds supplied by AppKit after an asynchronous resize and lays out the countdowns right-to-left from the trailing icon slot.

A press is classified against the drawn countdown frames, with a small hit expansion; gaps and the trailing icon slot open the menu. After a countdown press, visual status updates pause for the system double-click interval so the strings cannot move before the second press. A second press on the same countdown deletes it, while right-click deletes the countdown under the cursor and opens the menu elsewhere.

The app never assigns `NSStatusItem.menu` — on recent macOS that would hand all mouse handling to AppKit and break drag tracking. Instead the status button sends `leftMouseDown` to the app, which installs paired local and global `NSEvent` monitors for drag and mouse-up events. The band is visible from the first pixels of pull, red until the cursor can arm a timer; the pull only engages once the cursor leaves the menu bar by 10 px. Releasing while still inside the bar always opens the menu, so plain clicks stay reliable. A watchdog aborts tracking if the system swallows the mouse-up.

## Development

`MyTimer.app/Contents/MacOS/MyTimer --selftest` verifies the distance mapping, formatting, and manual-entry parsing. With `MYTIMER_DEBUG=1` in the app's environment, it logs state changes to `$TMPDIR/mytimer-debug.log`, and `--send-add-seconds N`, `--send-delete-id <uuid-prefix>`, `--send-clear`, and `--send-write-frame` drive a running instance through distributed notifications.
